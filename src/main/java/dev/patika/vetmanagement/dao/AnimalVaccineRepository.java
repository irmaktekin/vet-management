...
public interface AnimalVaccineRepository extends JpaRepository<AnimalVaccine,Long> {
    ...
    @Query("SELECT av FROM AnimalVaccine av WHERE av.animal.id = :animalId AND av.vaccine.id = :vaccineId AND av.vaccine.protectionFinishDate > CURRENT_DATE")
    Optional<AnimalVaccine> findActiveAnimalVaccinesByVaccineId(@Param("animalId") Long animalId, @Param("vaccineId") Long vaccineId);
