...
public interface AppointmentRepo extends JpaRepository<Appointment,Integer> {
    ...
    @Query("SELECT a FROM Appointment a JOIN a.animal an WHERE an.name = :animalName AND a.appointmentDate BETWEEN :startDate AND :endDate")
    ...
    @Query("SELECT a FROM Appointment a JOIN a.animal an WHERE an.id = :animalId AND a.appointmentDate BETWEEN :startDate AND :endDate")
    List<Appointment> findAppointmentsByAnimalAndDateRange(@Param("animalId") Long animalId, @Param("startDate") LocalDateTime startDate, @Param("endDate") LocalDateTime endDate);

    @Query("SELECT a FROM Appointment a WHERE a.doctor.id = :doctorId AND a.appointmentDate BETWEEN :startDate AND :endDate")
    List<Appointment> findAppointmentsByDoctorAndDateRange(
